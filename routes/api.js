...
router.get('/v1', verifyApiKey, function(req, res) {
    res.send({success: true, message: 'Ganjapay API v1.0 Coming soon!'})
});
...
function verifyApiKey(req, res, next){
    ...
    db.collection('users').findOne({'apiKeys.ganjapay': req.query.apikey }, function(error, response){
         if(response){
            ...
            response = {
                success: false,
                message: 'Invalid Ganjapay API key, please verifiy you\'re using the correct public key and try again.'
            }
