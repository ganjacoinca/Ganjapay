        ...
        $.getJSON('/user/balance', function( data ) {
            ...
            if(data.success == false){
                ...
                $.each(data.data.addresses, function(){
                    ...
                    addressesBody += '<tr><td><div id="qrcode2" style="border-width: 0px;" ></div></td><td><a style="color: #222222" href="http://explorer.ganjapay.io/api/addr/'+data.data.addresses[i]+'" class="addressList" id="'+data.data.addresses[i]+'">'+data.data.addresses[i]+'</a><td></tr>';
                    label++;
                    i++;
                });
...
// Ganjacoin Transactions Functions ===========(STILL NEEDS REFACTOR)=====
    ...
    function ccnWithdrawForm(balance) {
       ...
       $('#withdrawFunds').colorbox({html:withdrawForm, top:'150px', title: 'Ganjacoin Withdraw'});
    };
            ...
            if (txInfo.success == false) {
                ...
                $.each(txInfo.tx, function(){
                    ...
                    if(this.address == 'CWZfS4viQ28mGCpeTEbMme2eZZFZ3SGtJE'){
                        tableContent += '<td class="userEmail"><a href="http://ganjacoin.cc:2750/address/'+this.address+'">Ganjapay Fee (1%)</a></td>';    
                    } else {
                            tableContent += '<td class="userEmail"><a href="http://ganjacoin.cc:2750/address/'+this.address+'">'+this.address+'</a></td>';    
                    }
                    if(this.amount < 0) {
                    ...
                    tableContent += '<td class="userEmail"><a href="http://ganjacoin.cc:2750/tx/'+this.txid+'">'+(this.txid).substring(0,16)+'...</a></td>';
    ...
    function populateUserSettings() {
        ...
        var userSettingsYubiKey = '';
            ...
            $.getJSON( '/user', function( data ) {
                ...
                userSettingsApi += '<input id="key" class="form" placeholder="Ganjapay API Key" name="ganjapay" value="'+data.apiKeys.ganjapay+'"><br>';
                ...
                userSettingsYubiKey += '<p align="left">Caution: Please enter your fObfuscate UUID above, you must insure that you have entered the correct UUID as once it is enabled it is required in order to authenticate with Ganjapay. If for any reason the UUID is incorrect you will be locked out of your account and will need to contact the site admin.</a>';
